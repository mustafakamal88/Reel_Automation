...
func (p *YouTubeProvider) AnalyzeChannel(ctx context.Context, channelURL string) (ChannelAnalysisResult, error) {
	...
	log.Printf("channel_analysis_stage=keywords_extracted channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	keywordIntel = sanitizeChannelKeywordIntelligence(keywordIntel, performanceSample, identity)
	log.Printf("channel_analysis_stage=keywords_sanitized channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	keywords := append(append([]string{}, keywordIntel.PrimaryKeywords...), keywordIntel.SecondaryKeywords...)
	nicheAnalysis := ClassifyNiche(KeywordExtractionInput{
		Title:             channel.Snippet.Title,
	...
	log.Printf("channel_analysis_stage=niche_classified channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	pillarObjects := buildChannelPillars(keywordIntel, performanceSample, identity)
	log.Printf("channel_analysis_stage=pillars_built channel_id=%s count=%d elapsed_ms=%d", channelID, len(pillarObjects), time.Since(startedAt).Milliseconds())
	pillars := topPillarNames(pillarObjects, 6)
	keywords = sanitizeChannelKeywordList(keywords, performanceSample, identity, 12)
	log.Printf("channel_analysis_stage=keywords_final channel_id=%s count=%d elapsed_ms=%d", channelID, len(keywords), time.Since(startedAt).Milliseconds())
	...
	log.Printf("channel_analysis_stage=pillars_assigned channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	...
	log.Printf("channel_analysis_stage=patterns_built channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	...
	log.Printf("channel_analysis_stage=ideas_built channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	...
	log.Printf("channel_analysis_stage=metrics_built channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
	...
	log.Printf("channel_analysis_stage=charts_packaging_built channel_id=%s elapsed_ms=%d", channelID, time.Since(startedAt).Milliseconds())
